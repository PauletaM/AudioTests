   ...
   public class UILineRenderer : UIPrimitiveBase
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            for (var i = 0; i < segments.Count; i++)
            {
                ...
                if ( m_points[i].x < GetComponent<Spectrum>().leftHandle.rectTrans.anchoredPosition.x )
                    ChangeColor(segments[i]);
        ...
        private void ChangeColor( UIVertex[] vert )
        {
            for ( int i = 0; i < vert.Length; i++ )
            {
                UIVertex v = new UIVertex();
                v.color = new Color( 1, 0, 0, 1 );
                v.position = vert[ i ].position;
                v.normal = vert[ i ].normal;
                v.tangent = vert[ i ].tangent;
                v.uv0 = vert[ i ].uv0;
                vert[ i ] = v;
            }
        }        
